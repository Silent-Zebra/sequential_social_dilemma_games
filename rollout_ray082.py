...
def run(args, parser):
    model_name_l = "conv_to_fc_net_large"
    ModelCatalog.register_custom_model(model_name_l, ConvToFCNetLarge)
    ...
    if args.env == 'harvest':
        def env_creator(_):
            ...
            if args.harvest_map == 'tiny':
                ascii_map = HARVEST_MAP_TINY
            elif args.harvest_map == 'toy':
                ascii_map = HARVEST_MAP_TOY
            elif args.harvest_map == 'cpr':  # note only single agent at present
                ascii_map = HARVEST_MAP_CPR
            elif args.harvest_map == 'big':
                ascii_map = HARVEST_MAP_BIG
            created_env = HarvestEnv(ascii_map=ascii_map, num_agents=args.num_agents,
                                     )
            return created_env
        ...
        def env_creator(_):
            ...
            if args.cleanup_map == 'small':
                ascii_map = CLEANUP_MAP_SMALL
            created_env = CleanupEnv(ascii_map=ascii_map, num_agents=args.num_agents,
                                     )
            return created_env
